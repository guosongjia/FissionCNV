...
if __name__ == "__main__":
    ...
    inputFaiFile = sys.argv[2]
    outputVcfFile = sys.argv[3]
    ...
    print('##INFO=<ID=GCF,Number=1,Type=Float,Description="GC fraction">', file=Out)
    print('##FORMAT=<ID=CN,Number=1,Type=Integer,Description="Integer copy number">', file=Out)
    print('##FORMAT=<ID=AS,Number=1,Type=Float,Description="Accumulated overlapped fraction/score">', file=Out)
    ...
    with open(inputBedFile, 'r') as f:
        for x in f:
            ...
            x = x.strip().split('\t')
            chrom = x[0]
            pos = int(x[1]) + 1  # BED is 0-based, VCF is 1-based
            end = int(x[2])
            cn = int(x[3])
            tools = x[4]
            toolNum = int(x[5])
